...
fn fill_vertices(@builtin(global_invocation_id) gid: vec3<u32>) {
    switch (gid.x) {
        case 0u: {
            ...
            usl_g0_b0.values[gid.x].color = vec4<f32>(1.0, 0.18, 0.08, 1.0);
        }
        case 1u: {
            usl_g0_b0.values[gid.x].position = vec4<f32>(0.68, -0.62, 0.0, 1.0);
            usl_g0_b0.values[gid.x].color = vec4<f32>(0.12, 1.0, 0.32, 1.0);
        }
        default: {
            usl_g0_b0.values[gid.x].position = vec4<f32>(0.0, 0.7, 0.0, 1.0);
            usl_g0_b0.values[gid.x].color = vec4<f32>(0.12, 0.38, 1.0, 1.0);
        }
    }
    return;
}
