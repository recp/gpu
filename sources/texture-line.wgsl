...
fn line_transform_cs(@builtin(global_invocation_id) usl_builtin_2: vec3<u32>) {
    ...
    let r26: vec4<f32> = vec4<f32>(r12, r12, r12, 1.0);
    let r27: vec4<f32> = (vec4<f32>(r14[2], r14[0], r14[1], 1.0) * r26);
...
fn line_vs(@builtin(vertex_index) vertexId: u32) -> LineOut {
    switch (vertexId) {
        case 0u: {
            return LineOut(vec4<f32>(-0.88, -0.76, 0.0, 1.0), vec2<f32>(0.0, 1.0));
        }
        case 1u: {
            return LineOut(vec4<f32>(0.88, -0.76, 0.0, 1.0), vec2<f32>(1.0));
        }
        case 2u: {
            return LineOut(vec4<f32>(-0.88, 0.76, 0.0, 1.0), vec2<f32>(0.0));
        }
        case 3u: {
            return LineOut(vec4<f32>(-0.88, 0.76, 0.0, 1.0), vec2<f32>(0.0));
        }
        ...
        default: {
            return LineOut(vec4<f32>(0.88, 0.76, 0.0, 1.0), vec2<f32>(1.0, 0.0));
        }
    }
}

@fragment
fn line_fs(input: LineOut) -> @location(0) vec4<f32> {
