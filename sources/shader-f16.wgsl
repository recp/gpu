...
fn fill_f16_vertices(@builtin(global_invocation_id) gid: vec3u) {
    let r16 = f32(fma(0.27001953, f16(gid.x), 0.41992188));
    let r19: bool = (gid.x == 0u);
    let r22: bool = (gid.x == 1u);
    let r41 = select(vec4f(0.12, r16, 1.0, 1.0), vec4f(r16, 1.0, 0.3, 1.0), r22);
    usl_g0_b0.values[gid.x].position = vec4f(select(select(0.0, 0.68, r22), -0.68, r19), select(-0.62, 0.7, (gid.x == 2u)), 0.0, 1.0);
    let r55 = (select(r41, vec4f(1.0, r16, 0.08, 1.0), r19) * usl_g3_b0.value.tint);
