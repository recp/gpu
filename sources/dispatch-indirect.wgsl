...
fn fill_indirect_vertices(@builtin(global_invocation_id) gid: vec3<u32>) {
    switch (gid.x) {
        case 0u: {
            usl_g0_b0.values[gid.x].position = vec4<f32>(-0.68, -0.62, 0.0, 1.0);
            usl_g0_b0.values[gid.x].color = (vec4<f32>(1.0, 0.4, 0.04, 1.0) * usl_g3_b0.value.tint);
        }
        case 1u: {
            usl_g0_b0.values[gid.x].position = vec4<f32>(0.68, -0.62, 0.0, 1.0);
            usl_g0_b0.values[gid.x].color = (vec4<f32>(0.04, 0.88, 0.58, 1.0) * usl_g3_b0.value.tint);
        }
        default: {
            usl_g0_b0.values[gid.x].position = vec4<f32>(0.0, 0.7, 0.0, 1.0);
            usl_g0_b0.values[gid.x].color = (vec4<f32>(0.12, 0.46, 1.0, 1.0) * usl_g3_b0.value.tint);
        }
    }
}
