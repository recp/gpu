...
fn fill_subgroup_vertices(@builtin(global_invocation_id) gid: vec3u) {
    let r10: f32 = fma(0.18, f32((gid.x & 3u)), 0.28);
    let r13: f32 = subgroupShuffleXor(r10, 1u);
    let r16: bool = (gid.x == 0u);
    let r19: bool = (gid.x == 1u);
    usl_g0_b0.values[gid.x].position = vec4f(select(select(0.0, 0.68, r19), -0.68, r16), select(-0.62, 0.7, (gid.x == 2u)), 0.0, 1.0);
    usl_g0_b0.values[gid.x].color = (select(select(vec4f(0.12, r13, 1.0, 1.0), vec4f(r13, 1.0, 0.3, 1.0), r19), vec4f(1.0, r13, 0.08, 1.0), r16) * usl_g3_b0.value.tint);
}
