...
GPU_HIDE bool dx12_createExperimentalFactory(
  uint32_t                featureCount,
  const IID              *features,
  ID3D12DeviceFactory   **outFactory);
GPU_HIDE HRESULT dx12_createNativeDevice(ID3D12DeviceFactory *factory,
                                         IUnknown             *adapter,
                                         REFIID                iid,
                                         void                **outDevice);
...
GPU_HIDE void dx12_encodeBarriers(GPUCommandBuffer       *cmdb,
                                  const GPUBarrierBatch *barriers);
GPU_HIDE void dx12_blitTexture(GPUCommandBuffer         *cmdb,
