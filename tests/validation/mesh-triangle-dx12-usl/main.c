...
#include "../../../samples/common/MeshTriangle.h"
#include "../../../samples/common/SampleStats.h"

#define WIN32_LEAN_AND_MEAN
#include <windows.h>

#include "../../../samples/common/Win32Sample.h"
...
typedef struct MeshTriangleApp {
  ...
  HWND                      window;
  GPUSampleMeshTriangle    renderer;
  uint32_t                  width;
  uint32_t                  height;
...
static MeshTriangleApp *mesh_triangle_app;

static void
mesh_triangle_log(const char *message) {
  fprintf(stderr, "GPU DX12 mesh triangle: %s\n", message);
}

static bool
mesh_triangle_loadArtifact(void **outData, uint64_t *outSize) {
...
static bool
mesh_triangle_createWindow(MeshTriangleApp *app, HINSTANCE instance) {
  ...
  windowClass.lpszClassName = L"GPUUSLDX12MeshTriangle";
  ...
  app->window = CreateWindowExW(0,
                                ...
                                L"GPU + USL Direct3D 12 Mesh Triangle",
                                WS_OVERLAPPEDWINDOW,
                                CW_USEDEFAULT,
                                CW_USEDEFAULT,
...
static bool
mesh_triangle_createGPU(MeshTriangleApp *app) {
  ...
  instanceInfo.label            = "mesh-triangle-dx12-usl";
  ...
  if (GPUCreateDevice(app->adapter, &deviceInfo, &app->device) != GPU_OK ||
      !app->device ||
      !GPUIsFeatureEnabled(app->device, GPU_FEATURE_MESH_SHADER)) {
    return false;
  }
  ...
  artifact     = NULL;
  artifactSize = 0u;
  if (!mesh_triangle_loadArtifact(&artifact, &artifactSize)) {
    return false;
  }
  library = NULL;
  ...
  result = GPUSampleMeshTriangleInit(&app->renderer,
                                     app->device,
                                     app->queue,
                                     app->swapchain,
                                     library,
...
static bool
mesh_triangle_render(MeshTriangleApp *app) {
  GPUResult result;

  result = GPUSampleMeshTriangleRender(&app->renderer, NULL, NULL);
  if (result != GPU_OK) {
    return false;
  }
...
static bool
mesh_triangle_waitForGPU(MeshTriangleApp *app) {
  ...
  fenceInfo.label            = "mesh-triangle-dx12-shutdown";
...
static void
mesh_triangle_destroyGPU(MeshTriangleApp *app) {
  GPUSampleMeshTriangleDestroy(&app->renderer);
...
mesh_triangle_windowProc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) {
  switch (message) {
    case WM_CLOSE:
      if (mesh_triangle_app) {
        mesh_triangle_app->running = false;
      }
    ...
    case WM_DESTROY:
      ...
      return 0;
    case WM_SIZE:
      if (mesh_triangle_app && mesh_triangle_app->ready &&
        ...
        if (width > 0u && height > 0u &&
            GPUResizeSwapchain(mesh_triangle_app->swapchain,
                               width,
                               height) == GPU_OK &&
            GPUSampleMeshTriangleResize(&mesh_triangle_app->renderer,
                                        width,
                                        height) == GPU_OK) {
          mesh_triangle_app->width  = width;
          mesh_triangle_app->height = height;
        } else {
          mesh_triangle_app->running = false;
        }
      }
...
main(void) {
  ...
  mesh_triangle_app = &app;
  if (!mesh_triangle_createWindow(&app, instance)) {
    mesh_triangle_log("window creation failed");
    return 1;
  }

  previousProc = (WNDPROC)SetWindowLongPtrW(app.window,
                                            GWLP_WNDPROC,
                                            (LONG_PTR)mesh_triangle_windowProc);
  if (!previousProc || !mesh_triangle_createGPU(&app)) {
    mesh_triangle_log("GPU initialization failed");
    mesh_triangle_destroyGPU(&app);
  ...
  while (app.running) {
    ...
    if (app.running && !mesh_triangle_render(&app)) {
      mesh_triangle_log("frame rendering failed");
      result      = 1;
      app.running = false;
    }
  }

  if (!mesh_triangle_waitForGPU(&app)) {
    mesh_triangle_log("queue drain failed");
    result = 1;
  }
  mesh_triangle_destroyGPU(&app);
