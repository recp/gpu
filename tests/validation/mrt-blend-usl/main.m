...
#import "../../../samples/common/MRTBlend.h"
#import "../../../samples/common/SampleApp.h"
#import "../../../samples/common/SampleStats.h"
#import "../../../samples/common/SampleUSL.h"
...
static NSString *
MRTBlendWindowTitle(void) {
  return GPU_SAMPLE_BACKEND == GPU_BACKEND_VULKAN
           ? @"GPU Vulkan USL MRT Blend"
           : @"GPU Metal USL MRT Blend";
}

static const char *
MRTBlendStatsLabel(void) {
  return GPU_SAMPLE_BACKEND == GPU_BACKEND_VULKAN
           ? "GPU Vulkan MRT blend"
           : "GPU Metal MRT blend";
}

@interface MRTBlendApp : NSObject <NSApplicationDelegate, NSWindowDelegate> {
@private
  NSWindow          *_window;
  NSView            *_view;
...
static void
MRTBlendFrameComplete(void *sender, GPUCommandBuffer *cmdb) {
  MRTBlendApp *app;

  (void)cmdb;
  app = (__bridge MRTBlendApp *)sender;
  [app frameCompleted];
}

static void
MRTBlendDeviceError(GPUDevice                *device,
                    const GPUDeviceErrorInfo *error,
                    void                     *userData) {
  MRTBlendApp *app;

  (void)device;
  app = (__bridge MRTBlendApp *)userData;
  NSLog(@"GPU MRT error: %s", error && error->message ? error->message
                                                      : "unknown error");
  if (app) {
...
- (BOOL)setupGPU {
  ...
  instanceInfo.label            = "mrt-blend-native-usl";
  ...
  return GPUSampleMRTBlendInit(&_renderer,
                               _device,
                               _queue,
                               _swapchain,
                               library,
...
- (void)renderFrame {
  ...
  if (GPUSampleMRTBlendResize(&_renderer, width, height) != GPU_OK) {
    return;
  }

  completion = _exitAfterFrames > 0 ? MRTBlendFrameComplete : NULL;
  result = GPUSampleMRTBlendRender(&_renderer,
                                   (__bridge void *)self,
                                   completion);
  if (result != GPU_OK) {
    NSLog(@"GPU MRT frame failed: %d", result);
    return;
  }
...
- (void)cleanupGPU {
  GPUSampleMRTBlendDestroy(&_renderer);
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
  ...
  (void)notification;
  if (!GPUSampleCreateWindow(MRTBlendWindowTitle(), self, &_window, &_view) ||
      ![self setupGPU]) {
    [NSApp terminate:nil];
    return;
  }
...
main(int argc, const char *argv[]) {
  ...
  result = 0;
  @autoreleasepool {
    MRTBlendApp *delegate;

    (void)argc;
    (void)argv;
    [NSApplication sharedApplication];
    delegate = [MRTBlendApp new];
