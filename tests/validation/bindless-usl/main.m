...
#import "../../../samples/common/BindlessTexture.h"
...
@private
  ...
  NSTimer                  *_timer;
  GPUSampleBindlessTexture  _renderer;
...
BindlessTextureFrameComplete(void *sender, GPUCommandBuffer *cmdb) {
  BindlessTextureApp *app;

  (void)cmdb;
  app = (__bridge BindlessTextureApp *)sender;
  [app frameCompleted];
}

static void
BindlessTextureDeviceError(GPUDevice                *device,
...
@implementation BindlessTextureApp
...
- (void)cleanupGPU {
  GPUSampleBindlessTextureDestroy(&_renderer);
...
main(int argc, const char *argv[]) {
  ...
  @autoreleasepool {
    ...
    delegate = [BindlessTextureApp new];
