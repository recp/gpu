...
ARTIFACT_DIR="$BUILD_DIR/tests/usl/metal"
DEFAULT_ARTIFACT_DIR="$ARTIFACT_DIR/validation"
SMOKE_ARTIFACT_DIR="$ARTIFACT_DIR/smoke"
TRIANGLE_MANUAL_BIN="$BUILD_DIR/validation/gpu-triangle-manual/gpu-triangle-manual"
TRIANGLE_USL_BIN="$BUILD_DIR/validation/gpu-triangle-metal-usl/gpu-triangle-metal-usl"
TEXTURED_QUAD_BIN="$BUILD_DIR/validation/gpu-textured-quad-metal-usl/gpu-textured-quad-metal-usl"
MESH_TRIANGLE_BIN="$BUILD_DIR/validation/gpu-mesh-triangle-metal-usl/gpu-mesh-triangle-metal-usl"
COMPUTE_USL_BIN="$BUILD_DIR/validation/gpu-compute-metal-usl/gpu-compute-metal-usl"
COMPUTE_BUFFER_BIN="$BUILD_DIR/validation/gpu-compute-buffer-metal-usl/gpu-compute-buffer-metal-usl"
...
run_binary() {
  ...
  (
    cd "$BUILD_DIR/validation/$target"
    "./$target" "$@"
  )
}
...
install_artifact() {
  ...
  local target="$2"
  local name="$3"
  local targetDir="$BUILD_DIR/validation/$target"
...
run_step "triangle-usl sidecar" \
  generate_artifact sidecar "$ROOT/tests/validation/triangle-usl/triangle.usl"
run_step "triangle-usl embedded" \
  generate_artifact embedded "$ROOT/tests/validation/triangle-usl/triangle.usl"
...
run_step "textured-quad-usl sidecar" \
  generate_artifact sidecar \
    "$ROOT/tests/validation/textured-quad-usl/textured_quad.usl"
...
run_step "compute-usl sidecar" \
  generate_artifact sidecar "$ROOT/tests/validation/compute-usl/compute_visible.usl"
run_step "compute-usl embedded" \
  generate_artifact embedded "$ROOT/tests/validation/compute-usl/compute_visible.usl"
install_artifact \
...
run_step "compute-buffer-usl sidecar" \
  generate_artifact sidecar \
    "$ROOT/tests/validation/compute-buffer-usl/compute_buffer.usl"
...
run_step "usl-reflection-check embedded reflection" \
  generate_artifact embedded \
    "$ROOT/tests/validation/usl-reflection-check/reflection.usl"
run_step "usl-reflection-check embedded storage" \
  generate_artifact embedded \
    "$ROOT/tests/validation/usl-reflection-check/reflection_storage.usl"
