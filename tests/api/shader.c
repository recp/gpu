...
static int
check_reflected_bind_group_layout(GPUBindGroupLayout *layout) {
  const GPUBindGroupLayoutEntry *entries;
  uint32_t count;

  entries = GPUGetBindGroupLayoutEntries(layout, &count);
  if (count != 2u) {
    fprintf(stderr, "unexpected reflected bind group layout entry count\n");
    return 0;
  }

  return layout_has_typed_entry(entries,
                                count,
...
static int
check_selected_shader_library(GPUDevice *device,
                              const void *bytecode,
  ...
  if (ok) {
    ...
    ok = GPUCreateBindGroupLayoutsFromReflection(device,
                                                 library,
                                                 &layoutCount,
                                                 layouts) == GPU_ERROR_INSUFFICIENT_CAPACITY &&
         layoutCount == 1u;
  }
  ...
  if (ok) {
    ok = GPUCreateBindGroupLayoutsFromReflection(device,
         ...
         layoutCount == 1u &&
         layouts[0] != NULL &&
         check_reflected_bind_group_layout(layouts[0]) &&
...
gpu_test_shader(GPUDevice *device,
  ...
  ok = check_selected_shader_library(device,
                                     bytecode,
                                     bytecodeSize,
                                     expectedSourceKind) &&
