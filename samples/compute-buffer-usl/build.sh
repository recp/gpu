...
rm -f "$SAMPLE_DIR/compute_buffer.usl.metal"

ustest_cmd=("$USTEST" --shader "$SAMPLE_DIR/compute_buffer.usl" --no-logs)
if [[ "$NO_SIDECAR" == "1" ]]; then
  ustest_cmd+=(--no-sidecar)
fi

if [[ "$EMBED_METAL" == "1" ]]; then
  USL_EMBED_METAL_BLOB=1 "${ustest_cmd[@]}" >/tmp/gpu-compute-buffer-usl-ustest.log 2>&1
else
  "${ustest_cmd[@]}" >/tmp/gpu-compute-buffer-usl-ustest.log 2>&1
fi

if [[ ! -f "$SAMPLE_DIR/compute_buffer.us" ]]; then
  echo "USL bytecode artifact was not generated: $SAMPLE_DIR/compute_buffer.us" >&2
  exit 1
fi

if [[ "$NO_SIDECAR" == "1" ]]; then
  if [[ -f "$SAMPLE_DIR/compute_buffer.usl.metal" ]]; then
    echo "USL Metal sidecar should not be generated in --no-sidecar mode" >&2
    exit 1
  fi
else
  if [[ ! -f "$SAMPLE_DIR/compute_buffer.usl.metal" ]]; then
    echo "USL Metal sidecar was not generated: $SAMPLE_DIR/compute_buffer.usl.metal" >&2
    exit 1
  fi
fi
